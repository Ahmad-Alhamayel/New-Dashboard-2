document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateCountdown() {
    ...
    if (difference < 0) {
      ...
      Object.values(timeElements).forEach(
...
window.onload = function () {
  document
    ...
    .addEventListener("click", loadDataAndFilter);
};
...
function getFirstDataSet(filteredData) {
  ...
  document.getElementById("percent1").innerHTML =
    (topicArray?.find((x) => x.topic == "Election Integrity")?.percent ?? 0) +
    "%";
  document.getElementById("count1").innerHTML =
    "(" +
    (topicArray?.find((x) => x.topic == "Election Integrity")?.count ?? 0) +
    ")";

  document.getElementById("percent2").innerHTML =
    (topicArray?.find((x) => x.topic == "Charisma")?.percent ?? 0) + "%";
  document.getElementById("count2").innerHTML =
    "(" + (topicArray?.find((x) => x.topic == "Charisma")?.count ?? 0) + ")";

  document.getElementById("percent3").innerHTML =
    (topicArray?.find((x) => x.topic == "Economy")?.percent ?? 0) + "%";
  document.getElementById("count3").innerHTML =
    "(" + (topicArray?.find((x) => x.topic == "Economy")?.count ?? 0) + ")";

  document.getElementById("percent4").innerHTML =
    (topicArray?.find((x) => x.topic == "Foreign Policy")?.percent ?? 0) + "%";
  document.getElementById("count4").innerHTML =
    "(" +
    (topicArray?.find((x) => x.topic == "Foreign Policy")?.count ?? 0) +
    ")";

  document.getElementById("percent5").innerHTML =
    (topicArray?.find((x) => x.topic == "Social Issues")?.percent ?? 0) + "%";
  document.getElementById("count5").innerHTML =
    "(" +
    (topicArray?.find((x) => x.topic == "Social Issues")?.count ?? 0) +
    ")";

  document.getElementById("percent6").innerHTML =
    (topicArray?.find((x) => x.topic == "Immigration")?.percent ?? 0) + "%";
  document.getElementById("count6").innerHTML =
    "(" + (topicArray?.find((x) => x.topic == "Immigration")?.count ?? 0) + ")";

  document.getElementById("percent7").innerHTML =
    (topicArray?.find((x) => x.topic == "Healthcare")?.percent ?? 0) + "%";
  document.getElementById("count7").innerHTML =
    "(" + (topicArray?.find((x) => x.topic == "Healthcare")?.count ?? 0) + ")";

  document.getElementById("percent8").innerHTML =
    (topicArray?.find((x) => x.topic == "Environment")?.percent ?? 0) + "%";
  document.getElementById("count8").innerHTML =
    "(" + (topicArray?.find((x) => x.topic == "Environment")?.count ?? 0) + ")";
}
...
function getResultChartDataSet(filteredData) {
  ...
  var TrampResult =
    ((TrampPositive + HarrisNegative) /
      (TrampPositive + TrampNegative + HarrisPositive + HarrisNegative)) *
    100;
  var HarrisResult =
    ((HarrisPositive + TrampNegative) /
      (TrampPositive + TrampNegative + HarrisPositive + HarrisNegative)) *
    100;
...
function getTopicsWithNumbersDataSet(filteredData) {
  ...
  const finalSentimentArray = Object.keys(sentimentArray).map((topic) => {
    ...
    const totalTrump =
      trumpCounts.Positive + trumpCounts.Negative + trumpCounts.Neutral;
    const totalHarris =
